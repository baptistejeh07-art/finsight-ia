...
def llm_select_and_summarize(candidates: list[dict]) -> dict:
    ...
    - Ecrit resumes editoriaux (3 phrases journalist style)
    - Redige intro editoriale 120-150 mots
    Retourne {editorial, selection:[{idx, resume, implication}]}
    """
  ...
  "selection": [
    ...
    try:
        ...
        if m:
            ...
            # Enrichir chaque selection avec les donnees originales
            enriched = []
...
def build_pdf(result: dict, bonus5: list[dict], output_path: Path) -> Path:
    ...
    if bonus5:
        ...
        for i, art in enumerate(bonus5):
            ...
            cat        = art.get("cat","IA")
            cat_color  = CAT_COLORS.get(cat, C_AMBER)
            source     = art.get("source","?")
            link       = art.get("link","")
            link_short = re.sub(r"^https?://(?:www\.)?", "", link)[:90]
