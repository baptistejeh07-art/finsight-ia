# -*- coding: utf-8 -*-
"""
build_lbo_sheet.py — Construit l'onglet LBO MODEL + _LBO_CALC dans un fichier Excel FinSight.

Approche : on travaille sur une copie d'un Excel généré (fichier vivant avec INPUT, DCF, etc.).
Une fois validé visuellement, le script sera intégré dans excel_writer.py et appliqué à
chaque génération d'analyse société.

Style : réplique fidèle de la charte FinSight existante (couleurs, polices, layout DCF).

Usage :
    python tools/build_lbo_sheet.py [path_to_xlsx]
    Default : assets/_LBO_WORKING_AAPL.xlsx
"""
